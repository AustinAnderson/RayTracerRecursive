...
Point calculateColor(SceneObject closestObject, Vector normalVector, Vector ray, Point isectWorldPoint) {
	...
	for (int i = 0; i < numLights; i++) {
		...
		Point diffuse(closestObject.material.cDiffuse.r,
        ...
        */
		...
		Point specular(closestObject.material.cSpecular.r,
        ...
        for (int j = 0; j<3; j++) {
            color[j] += diffuse[j] * dot_nl 
                      + (specular[j] * lightColor[j] * power)*shadow;
            if (color[j]>1) {
                color[j] = 1.0;
            }
        }
...
void callback_start(int id) {
	...
	for (int i = 0; i < pixelWidth; i++) {
		...
		printf("% 3f%% [rendering line % 3d] run time %f, eta %fs\
                                                                 \r",
